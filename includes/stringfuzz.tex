...
\transformer{} transforms \smt{} problems. Table~\ref{tbl:transformers} lists the several transformers and configurations available.
Note that transformers \textit{translate} and \textit{reverse} preserve problem satisfiability \cite{ifaz} in specific cases.
...
\vspace{-0.15in}
\subsection{\texttt{stringstats}}

This tool takes an \smt{} problem as input and outputs a number of properties.
Properties include the number of variables/literals, the max/median syntactic depth of expressions, and the max/median literal length. 
